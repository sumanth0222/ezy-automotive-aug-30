        ...
        if (mainlevel.Items) {
          mainlevel.Items.filter((items) => {
            ...
            if (location.pathname === '/nowa/preview/' || location.pathname === '/nowa/preview/') {
              location.pathname = '/nowa/preview/dashboard/dashboard2/';
            }
      ...
      >
        ...
        <PerfectScrollbar options={{ suppressScrollX: true }} className="hor-scroll">
          <div className="main-sidebar-header active">
            <NavLink className="header-logo active" to={`${import.meta.env.BASE_URL}dashboard/dashboard2/`}>
              {/* <img
                src={imagesData('logo')}
              ...
              {menuitems.map((Item, itemi) => (
                <Fragment key={itemi + Math.random() * 100}>
                  ...
                  {Item.Items.map((menuItem, i) => (
                    <li className={`slide ${menuItem.active ? "is-expanded" : ""} 
                     `} key={i}>
                      {menuItem.type === "link" ? (
